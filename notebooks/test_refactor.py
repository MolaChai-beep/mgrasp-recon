...
class RefactorTests(unittest.TestCase):
    ...
    def test_kspace_layout_and_tic_helpers(self):
        ...
        curve = analyzer.extract_voxel_tic(img_dyn, (1, 2), normalize=True)
        self.assertEqual(curve.shape, (3,))
        self.assertGreater(curve[-1], curve[0])

        roi_mask = np.zeros((2, 4), dtype=bool)
        roi_mask[:, 1:3] = True
        roi_curve = analyzer.extract_roi_mean_tic(img_dyn, roi_mask, normalize=False)
        self.assertEqual(roi_curve.shape, (3,))
        np.testing.assert_allclose(roi_curve, img_dyn[:, roi_mask].mean(axis=1))

    def test_tic_roi_summary_metrics_and_report(self):
        from mgrasp_recon import SegmentationResult, TicAnalyzer

        analyzer = TicAnalyzer()
        img_dyn = np.ones((6, 5, 5), dtype=np.float32)
        img_dyn[:, 1, 1] = np.array([1.0, 4.0, 2.0, 5.0, 3.0, 6.0], dtype=np.float32)
        img_dyn[:, 1, 2] = np.array([1.0, 3.0, 2.0, 4.0, 3.0, 5.0], dtype=np.float32)
        img_dyn[:, 3, 3] = np.array([1.0, 1.2, 1.3, 1.4, 1.45, 1.5], dtype=np.float32)
        img_dyn[:, 3, 4] = np.array([1.0, 1.1, 1.15, 1.2, 1.25, 1.3], dtype=np.float32)

        vascular_mask = np.zeros((5, 5), dtype=bool)
        vascular_mask[1, 1] = True
        vascular_mask[1, 2] = True
        tissue_mask = np.zeros((5, 5), dtype=bool)
        tissue_mask[3, 3] = True
        tissue_mask[3, 4] = True

        summary = analyzer.summarize_roi_tics(
            img_dyn,
            vascular_mask=vascular_mask,
            tissue_mask=tissue_mask,
            normalize=False,
        )
        self.assertEqual(summary["vessel_single"].shape, (6,))
        self.assertEqual(summary["vessel_roi"].shape, (6,))
        self.assertEqual(summary["tissue_roi"].shape, (6,))
        self.assertEqual(summary["vascular_mask_size"], 2)
        self.assertEqual(summary["tissue_mask_size"], 2)
        self.assertFalse(np.array_equal(summary["vessel_single"], summary["vessel_roi"]))

        metrics = analyzer.summarize_noise_metrics(
            img_dyn,
            vascular_mask=vascular_mask,
            tissue_mask=tissue_mask,
            normalize=False,
        )
        self.assertIn("single_voxel", metrics)
        self.assertIn("vessel_roi", metrics)
        self.assertIn("tissue_roi", metrics)
        self.assertLess(metrics["vessel_roi"]["high_freq_std"], metrics["single_voxel"]["high_freq_std"])
        self.assertGreater(metrics["vessel_roi"]["peak_enhancement"], metrics["tissue_roi"]["peak_enhancement"])

        segmentation = SegmentationResult(
            mean_img=img_dyn.mean(axis=0),
            peak_img=img_dyn.max(axis=0),
            std_img=img_dyn.std(axis=0),
            baseline_mean=img_dyn[:2].mean(axis=0),
            norm_early_enh=np.ones((5, 5), dtype=np.float32),
            brain_mask=np.ones((5, 5), dtype=bool),
            brain_core_mask=np.ones((5, 5), dtype=bool),
            brain_ring_mask=np.zeros((5, 5), dtype=bool),
            pca_roi_mask=np.ones((5, 5), dtype=bool),
            vascular_mask=vascular_mask,
            tissue_mask=tissue_mask,
            brain_mask_threshold=0.1,
            enhancement_threshold=0.2,
        )
        report = analyzer.create_noise_report(
            img_dyn,
            segmentation=segmentation,
            basis=np.ones((6, 2), dtype=np.float32),
            normalize=False,
            title_prefix="Test",
        )
        self.assertIn("roi_overlay", report["figures"])
        self.assertIn("tic_summary", report["figures"])
        self.assertIn("basis_curves", report["figures"])

    def test_tic_resolution_comparison_and_mask_validation(self):
        from mgrasp_recon import TicAnalyzer

        analyzer = TicAnalyzer()
        base = np.ones((5, 4, 4), dtype=np.float32)
        vascular_mask = np.zeros((4, 4), dtype=bool)
        vascular_mask[0, 0] = True
        ...
        series_by_label["128"]["img_dyn"][:, 0, 0] = np.array([1, 2, 3, 4, 5], dtype=np.float32)
        series_by_label["256"]["img_dyn"][:, 0, 0] = np.array([2, 4, 6, 8, 10], dtype=np.float32)

        comparison = analyzer.summarize_resolution_comparison(series_by_label, normalize=True)
        self.assertIn("128__vs__256", comparison["vessel_roi_correlations"])
        self.assertAlmostEqual(comparison["vessel_roi_correlations"]["128__vs__256"], 1.0, places=6)

        with self.assertRaises(ValueError):
            overlapping = tissue_mask.copy()
            overlapping[0, 0] = True
            analyzer.summarize_roi_tics(base, vascular_mask=vascular_mask, tissue_mask=overlapping)
